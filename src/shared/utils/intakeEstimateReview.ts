...
export function resolveIntakePersistFieldsForTakeoffLine(input: {
  ...
  // Persist generated minutes whenever labor is attributed to install-family resolution,
  // including branches where `laborOrigin` is set but flags disagree after matcher edits.
  const generatedLaborMinutes =
    row.laborOrigin === 'install_family' || row.pricingPreview?.laborFromInstallFamily
