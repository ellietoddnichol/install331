...
/**
 * `modifiers_v1.applies_to_categories` is conventionally a JSON-encoded array
 * (e.g. `'["Toilet Accessories","Lockers"]'`), but legacy / hand-imported rows
 * sometimes carry pipe- or comma-delimited strings. Accept all three.
 */
function parseAppliesToCategories(input: unknown): string[] {
...
legacyRouter.delete('/catalog/items/:id', async (req, res) => {
  try {
    ...
    await dbCatalogRun('UPDATE catalog_items SET active = 0 WHERE id = ?', [req.params.id]);
  ...
  try {
    ...
    res.json(
      rows.map((row) => ({
        ...
        appliesToCategories: parseAppliesToCategories(row.applies_to_categories),
...
legacyRouter.put('/catalog/modifiers/:id', async (req, res) => {
  const modRel = getCatalogModifiersReadTableName();
  const existing = await dbCatalogGet<any>(`SELECT * FROM ${modRel} WHERE id = ?`, [req.params.id]);
  ...
  const record = {
    ...
    modifierKey: String((input.modifierKey ?? existing.modifier_key) || '')
      ...
      .toUpperCase()
      .replace(/\s+/g, '_'),
    appliesToCategories: input.appliesToCategories ?? parseAppliesToCategories(existing.applies_to_categories),
...
legacyRouter.delete('/catalog/modifiers/:id', async (req, res) => {
  ...
  try {
    await dbCatalogRun('UPDATE modifiers_v1 SET active = 0, updated_at = ? WHERE id = ?', [
    ...
    await syncCatalogToGoogleSheetOptional('deactivate modifier', () =>
      upsertModifierInGoogleSheet({
        ...
        name: existing.name,
        description: existing.description != null ? String(existing.description) : '',
        appliesToCategories: parseAppliesToCategories(existing.applies_to_categories),
...
legacyRouter.get('/catalog/bundles', async (_req, res) => {
  try {
    ...
    const rows = await dbCatalogAll<any>(`SELECT * FROM ${bundlesTable} ORDER BY bundle_name`);
    res.json(
      rows.map((row) => ({
        id: row.id,
...
legacyRouter.put('/catalog/bundles/:id', async (req, res) => {
  ...
  const existing = await dbCatalogGet<any>(`SELECT * FROM ${bundlesTable} WHERE id = ?`, [req.params.id]);
  ...
  const input = parsed.data;
  const now = new Date().toISOString();
  const bundleItems = await dbCatalogAll<{ sku: string | null }>(
    `SELECT sku FROM ${bundleItemsTable} WHERE bundle_id = ? ORDER BY sort_order, id`,
    [req.params.id]
  );
  const record = {
  ...
  try {
    await dbCatalogRun(
      'UPDATE bundles_v1 SET bundle_name = ?, category = ?, active = ?, updated_at = ? WHERE id = ?',
      [record.bundleName, record.category, record.active ? 1 : 0, now, record.bundleId]
    );
...
legacyRouter.delete('/catalog/bundles/:id', async (req, res) => {
  ...
  try {
    await dbCatalogRun('UPDATE bundles_v1 SET active = 0, updated_at = ? WHERE id = ?', [
      new Date().toISOString(),
      req.params.id,
    ]);
