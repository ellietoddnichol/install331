...
test('install-family fallback applies when catalog match has zero labor minutes', async () => {
  const { buildIntakeEstimateDraft } = await import('./intakeMatcherService.ts');
  const catalogItems: CatalogItem[] = [
    {
      id: 'item-zero-labor',
      ...
      adaFlag: false,
      active: true,
      installLaborFamily: 'partition_compartment',
    },
  ];
  const line = baseReviewLine({
    reviewLineFingerprint: 'fp-zl',
    matchedCatalogItemId: 'item-zero-labor',
    catalogMatch: {
      catalogItemId: 'item-zero-labor',
      sku: 'ZLAB-1',
      description: 'Partition door',
      category: 'Partitions',
      unit: 'EA',
    ...
    installScopeType: 'partition_hdpe_compartment',
    installFamilyFallback: { key: 'partition_compartment', minutes: 45, basis: 'per_compartment' },
  });
  const draft = await buildIntakeEstimateDraft({
    reviewLines: [line],
    catalog: catalogItems,
  ...
  assert.ok(draft);
  const sug = draft!.lineSuggestions[0];
  assert.equal(sug.laborOrigin, 'install_family');
  assert.equal(sug.pricingPreview?.laborFromInstallFamily, true);
  assert.equal(sug.pricingPreview?.installFamilyKey, 'partition_compartment');
  assert.ok(sug.pricingPreview && sug.pricingPreview.laborMinutesEach > 0);
});
